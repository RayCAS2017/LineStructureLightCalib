function [vl_light_inters, light_cb_intersects_cord, light_insects] = get_info_in_single_frame_undistort(imageFileName,cameraParas, is_show)
% cameraParas,matlab标定的参数数据格式
% Kc,内参矩阵[fx,0,cx;0,fy,cy;0,0,1];
% D，畸变系数[k1,k2,k3,p1,p2]
Kc = cameraParas.Intrinsics.IntrinsicMatrix';
D = cat(2,cameraParas.Intrinsics.RadialDistortion, cameraParas.Intrinsics.TangentialDistortion);
...
%****获取畸变矫正后的点坐标*********
% imagePoints1_u = undistortImagePoint(imagePoints1, Kc, D);
imagePoints1_u = undistortPoints(imagePoints1,cameraParas);
...
im0=im;
%****获取畸变矫正的图片*********
% im0_u = undistortImage(im0, Kc, D);
[im0_u, im0_u_newOrigin] = undistortImage(im0, cameraParas, 'OutputView','full');
...
for i = 1:size(line_horizon_index)    
    ...
    line_cur_points = imagePoints1_u(line_cur_points_index,:);
    x = line_cur_points(:,1);
    y = line_cur_points(:,2);
...
if is_show
    f3 = figure('Name','Detect undistored checkboard lines and vanish points');
    imshow(im0_u);
    hold on 
    xx=1:1:n;
    for i = 1:size(line_horizon_index)
        yy = line_horizon_paras(i,1)*xx + line_horizon_paras(i,2);
        plot(yy-im0_u_newOrigin(1), xx-im0_u_newOrigin(2), 'blue')
        hold on
    end
    for i = 1:size(line_vertical_index)
        yy = line_vertical_paras(i,1)*xx + line_vertical_paras(i,2);
        plot(yy-im0_u_newOrigin(1), xx-im0_u_newOrigin(2), 'green')
        hold on
    end
end
...
y1 = y(in);
% *****畸变矫正
points_light_tmp = [x1,y1];
% points_light_tmp_u = undistortImagePoint(points_light_tmp,Kc,D);
points_light_tmp_u = undistortPoints(points_light_tmp,cameraParas);
x1 = points_light_tmp_u(:,1);
y1 = points_light_tmp_u(:,2);
%3、拟合激光线
...
if is_show
    ...
    %im_u = undistortImage(im,Kc,D);
    [im_u, im_u_newOrigin] = undistortImage(im,cameraParas,'OutputView','full');
%     vertices_u = undistortImagePoint(vertices,Kc,D);
    vertices_u = undistortPoints(vertices,cameraParas);
    subplot(121)
    imshow(im_u);
    hold on 
%     for i = 1:4
...
%         hold on
%     end
    plot(vertices_u(:,2)-im_u_newOrigin(1),vertices_u(:,1)-im_u_newOrigin(2),'r','LineWidth',1.5);
    plot(x(in)-im_u_newOrigin(1),y(in)-im_u_newOrigin(2),'g+');
    hold on;
    plot(x(~in)-im_u_newOrigin(1),y(~in)-im_u_newOrigin(2),'bo');
    hold on
    xx=1:1:n;
    yy = line_light(1)*xx + line_light(2);
    plot(yy-im_u_newOrigin(1), xx-im_u_newOrigin(2),'red')
    subplot(122)
    imshow(BW);
end
...
if is_show
    figure(f3);
    hold on
    plot(vp_h(2)-im0_u_newOrigin(1),vp_h(1)-im0_u_newOrigin(2),'*','Color','black', 'MarkerSize', 10)
    hold on
    plot(vp_v(2)-im0_u_newOrigin(1),vp_v(1)-im0_u_newOrigin(2),'*','Color','black', 'MarkerSize', 10)
    hold on
    plot([vp_h(2)-im0_u_newOrigin(1), vp_v(2)-im0_u_newOrigin(1)],[vp_h(1)-im0_u_newOrigin(2), vp_v(1)-im0_u_newOrigin(2)],'Color','black')
    hold on
    plot(vl_light_inters(2)-im0_u_newOrigin(1),vl_light_inters(1)-im0_u_newOrigin(2),'*','Color','red', 'MarkerSize', 10)
    hold on
    light_x = n;
    light_y = line_light(1)*light_x + line_light(2);
    plot_light_point = [light_x, light_y];
    plot([vl_light_inters(2)-im0_u_newOrigin(1), plot_light_point(2)-im0_u_newOrigin(1)],[vl_light_inters(1)-im0_u_newOrigin(2), plot_light_point(1)-im0_u_newOrigin(2)],'Color','black')
end
...
if is_show
    f5 = figure('Name', 'Undistort Checkerboard corners and light intersects');
    imshow(im0_u);
    hold on
    for i=1:size(line_vertical_paras,1)
        plot(light_insects(i,2)-im0_u_newOrigin(1),light_insects(i,1)-im0_u_newOrigin(2),'r*', 'MarkerSize', 10);
        hold on;
        text(light_insects(i,2)-im0_u_newOrigin(1), light_insects(i,1)-im0_u_newOrigin(2), strcat(num2str(i),'\rightarrow'), 'HorizontalAlignment', 'right', 'FontSize',15, 'Color', 'red');
        hold on;
        for j = 1:size(line_horizon_paras,1)
            plot(cb_corners(i,j,2)-im0_u_newOrigin(1),cb_corners(i,j,1)-im0_u_newOrigin(2),'g+', 'MarkerSize', 10);
            hold on;
            text(cb_corners(i,j,2)-im0_u_newOrigin(1), cb_corners(i,j,1)-im0_u_newOrigin(2), strcat('(',num2str(i),',',num2str(j),')','\rightarrow'), 'HorizontalAlignment', 'right', 'FontSize',15, 'Color', 'green');
            hold on;
        end
    end
